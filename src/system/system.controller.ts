...
export class SystemController {
  ...
  @Post('notifications') saveNotifications(@Req() req: FastifyRequest, @Body() body: NotificationDto) { const session = this.auth.requireAdmin(req); const result = this.system.saveNotificationSettings(body); this.system.audit(session.user_id, 'settings.notifications.update'); return result; }
  ...
  @Post('environment') saveEnvironment(@Req() req: FastifyRequest, @Body() body: EnvironmentDto, @Res() reply: FastifyReply) {
  ...
  @Post('export/full') fullExport(@Req() req: FastifyRequest, @Body() body: FullExportDto, @Res() reply: FastifyReply) { const session = this.auth.requireAdmin(req); const result = this.system.fullExport(String(body.password || '')); this.system.audit(session.user_id, 'config.full_export'); reply.header('Content-Disposition', 'attachment; filename="vaultback-encrypted-export.json"'); reply.type('application/json'); return reply.send(JSON.stringify(result)); }
  @Post('import/full') fullImport(@Req() req: FastifyRequest, @Body() body: FullImportDto) { const session = this.auth.requireAdmin(req); const result = this.system.importFull(body.package, String(body.password || '')); this.system.audit(session.user_id, 'config.full_import'); return result; }
