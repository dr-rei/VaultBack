...
export class BackupService {
  ...
  private dumpWithArgs(connection: DatabaseConnection, output: string, dumpArgs: string[], processId?: string, includeDatabaseObjects = true, includeTriggers = true) { return new Promise<void>((resolve, reject) => { const binary = this.resolveClientBinary(connection.engine, true); const transportArgs = connection.ssl ? ['--ssl'] : ['--skip-ssl']; const args = [...this.clientToolArgs(binary), `--host=${connection.host}`, `--port=${connection.port}`, `--user=${connection.username}`, ...transportArgs, '--single-transaction', ...(includeDatabaseObjects ? ['--routines', '--events'] : []), ...(includeTriggers ? ['--triggers'] : []), '--hex-blob', ...dumpArgs]; const child = spawn(binary, args, { env: { ...process.env, MYSQL_PWD: connection.password }, stdio: ['ignore', 'pipe', 'pipe'], windowsHide: true }); const out = fs.createWriteStream(output, { mode: 0o600 }); let stderr = ''; child.stdout.pipe(out); child.stderr.on('data', chunk => { const text = chunk.toString(); stderr += text; if (processId) this.addProcessLog(processId, text); }); child.on('error', error => { out.close(); if (processId) this.addProcessLog(processId, this.clientStartMessage(binary, error)); reject(new Error(this.clientStartMessage(binary, error))); }); child.on('close', code => { out.close(); if (code === 0) resolve(); else reject(new Error(`${binary} failed with code ${code}: ${stderr.slice(0, 800)}`)); }); }); }
