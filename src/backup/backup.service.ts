...
export class BackupService {
  ...
  async listAvailableDatabases(id: string) {
   ...
   listJobs() { const rows = this.store.db.prepare('SELECT id,name,database_connection_id as databaseConnectionId,storage_target_id as storageTargetId,database_scope as databaseScope,database_names as databaseNames,backup_layout as backupLayout,table_schema_separate as tableSchemaSeparate,cron_expression as cronExpression,timezone,enabled,compression,backup_encryption as backupEncryption,retention_count as retentionCount,filename_prefix as filenamePrefix,next_run_at as nextRunAt,last_run_at as lastRunAt,created_at as createdAt FROM backup_jobs ORDER BY name').all() as any[]; return rows.map(row => { let databases: string[] = []; try { databases = JSON.parse(String(row.databaseNames || '[]')); } catch {} return { ...row, databaseScope: row.databaseScope === 'all' ? 'all' : 'selected', backupLayout: ['database', 'table'].includes(row.backupLayout) ? row.backupLayout : 'single', tableSchemaSeparate: Boolean(row.tableSchemaSeparate), databases }; }); }
   saveJob(body: any) { if (!body.name || !body.databaseConnectionId || !body.storageTargetId || !body.cronExpression) throw new BadRequestException('Name, database, storage, and schedule are required'); const databaseScope = body.databaseScope === 'all' ? 'all' : 'selected'; const rawDatabases = Array.isArray(body.databases) ? body.databases : String(body.databases ?? '').split(/[\n,]/); const databases = [...new Set(rawDatabases.map((value: unknown) => String(value).trim()).filter(Boolean))]; if (databaseScope === 'selected' && !databases.length) throw new BadRequestException('Select at least one database'); const backupLayout = ['database', 'table'].includes(body.backupLayout) ? body.backupLayout : 'single'; const tableSchemaSeparate = backupLayout === 'table' && (body.tableSchemaSeparate === true || body.tableSchemaSeparate === 'true' || body.tableSchemaSeparate === 1); const timezone = body.timezone || 'UTC'; nextCron(body.cronExpression, new Date(), timezone); const id = body.id || crypto.randomUUID(); const prefix = safeFilePart(body.filenamePrefix || body.name); const next = nextCron(body.cronExpression, new Date(), timezone).toISOString(); const encryption = body.backupEncryption === 'aes-256-gcm' ? 'aes-256-gcm' : 'none'; const requestedCompression = ['none', 'gzip', 'zip'].includes(body.compression) ? body.compression : 'gzip'; const compression = backupLayout === 'single' ? requestedCompression : 'zip'; this.store.db.prepare(`INSERT INTO backup_jobs (id,name,database_connection_id,storage_target_id,database_scope,database_names,backup_layout,table_schema_separate,cron_expression,timezone,enabled,compression,backup_encryption,retention_count,filename_prefix,next_run_at,last_run_at,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET name=excluded.name,database_connection_id=excluded.database_connection_id,storage_target_id=excluded.storage_target_id,database_scope=excluded.database_scope,database_names=excluded.database_names,backup_layout=excluded.backup_layout,table_schema_separate=excluded.table_schema_separate,cron_expression=excluded.cron_expression,timezone=excluded.timezone,enabled=excluded.enabled,compression=excluded.compression,backup_encryption=excluded.backup_encryption,retention_count=excluded.retention_count,filename_prefix=excluded.filename_prefix,next_run_at=excluded.next_run_at`).run(id, body.name, body.databaseConnectionId, body.storageTargetId, databaseScope, JSON.stringify(databases), backupLayout, tableSchemaSeparate ? 1 : 0, body.cronExpression, timezone, body.enabled === false ? 0 : 1, compression, encryption, Math.max(1, Math.min(365, Number(body.retentionCount || 7))), prefix, next, body.lastRunAt || null, this.store.now()); return this.listJobs().find((x: any) => x.id === id); }
  ...
  private async run(jobId: string) {
    ...
    try {
      const connection = this.getConnection(job.database_connection_id); const databaseScope: 'all' | 'selected' = job.database_scope === 'all' ? 'all' : 'selected'; const backupLayout = ['database', 'table'].includes(job.backup_layout) ? job.backup_layout : 'single'; const tableSchemaSeparate = backupLayout === 'table' && Number(job.table_schema_separate) === 1; let databases: string[] = []; try { databases = JSON.parse(String(job.database_names || '[]')); } catch {} if (databaseScope === 'selected' && !databases.length) databases = connection.databases; if (backupLayout !== 'single' && databaseScope === 'all') databases = (await this.listAvailableDatabases(connection.id)).databases; if (databaseScope === 'selected' && !databases.length) throw new Error('No databases selected for this schedule'); const backupConnection: DatabaseConnection = { ...connection, databaseScope, databases, database: databaseScope === 'all' ? '*' : databases[0] || '' }; const target = this.storage.get(job.storage_target_id); tempDir = fs.mkdtempSync(path.join(this.store.dataDir, 'tmp', 'run-')); const databaseLabel = backupConnection.databaseScope === 'all' ? 'all-databases' : backupConnection.databases.join('-'); const raw = path.join(tempDir, `${safeFilePart(databaseLabel || backupConnection.database)}-${Date.now()}.sql`); const compression = backupLayout === 'single' && ['none', 'gzip', 'zip'].includes(job.compression) ? job.compression : backupLayout === 'single' ? 'gzip' : 'zip'; let artifact = raw;
      this.setProcessStage(runId, 'dumping'); this.addProcessLog(runId, `Dumping ${backupLayout === 'single' ? (databaseScope === 'all' ? 'all visible databases' : `${databases.length} selected database(s)`) : backupLayout === 'database' ? `${databases.length} database file(s)` : tableSchemaSeparate ? 'one SQL file per table with separate schema files' : 'one SQL file per table'}`);
      if (backupLayout === 'single') { await this.dump(backupConnection, raw, runId); if (compression === 'gzip') { this.setProcessStage(runId, 'compressing'); this.addProcessLog(runId, 'Compressing SQL archive'); artifact = `${raw}.gz`; await pipeline(fs.createReadStream(raw), createGzip({ level: 6 }), fs.createWriteStream(artifact)); fs.rmSync(raw); } else if (compression === 'zip') { this.setProcessStage(runId, 'compressing'); this.addProcessLog(runId, 'Creating ZIP archive'); artifact = `${raw}.zip`; await this.writeZip([{ name: 'backup.sql', path: raw }], artifact); fs.rmSync(raw); } } else { const entries: Array<{ name: string; path: string }> = []; if (backupLayout === 'database') { for (const database of databases) { const output = path.join(tempDir, `${safeFilePart(database)}.sql`); await this.dumpDatabase(backupConnection, database, output, runId); entries.push({ name: `${safeFilePart(database)}/${safeFilePart(database)}.sql`, path: output }); } } else { for (const database of databases) { const tables = await this.listAvailableTables(connection, database); if (!tables.length) this.addProcessLog(runId, `No tables found in ${database}`); for (const table of tables) { const databasePart = safeFilePart(database); const tablePart = safeFilePart(table); if (tableSchemaSeparate) { const schemaOutput = path.join(tempDir, `${databasePart}-${tablePart}.schema.sql`); const dataOutput = path.join(tempDir, `${databasePart}-${tablePart}.data.sql`); await this.dumpTable(backupConnection, database, table, schemaOutput, runId, 'schema'); await this.dumpTable(backupConnection, database, table, dataOutput, runId, 'data'); entries.push({ name: `${databasePart}/${tablePart}.schema.sql`, path: schemaOutput }, { name: `${databasePart}/${tablePart}.data.sql`, path: dataOutput }); } else { const output = path.join(tempDir, `${databasePart}-${tablePart}.sql`); await this.dumpTable(backupConnection, database, table, output, runId); entries.push({ name: `${databasePart}/${tablePart}.sql`, path: output }); } } } } if (!entries.length) throw new Error('No databases or tables were available for the selected backup scope'); this.setProcessStage(runId, 'compressing'); this.addProcessLog(runId, `Creating ZIP archive with ${entries.length} SQL file(s)`); artifact = path.join(tempDir, `${safeFilePart(databaseLabel)}.zip`); await this.writeZip(entries, artifact); }
  ...
  private async listAvailableTables(connection: DatabaseConnection, database: string) {
    ...
    return [...new Set((rows as any[]).map(row => String(Object.values(row)[0] || '').trim()).filter(Boolean))];
  }
  ...
  private dumpTable(connection: DatabaseConnection, database: string, table: string, output: string, processId?: string, mode: 'combined' | 'schema' | 'data' = 'combined') { const dumpArgs = mode === 'schema' ? ['--no-data', database, table] : mode === 'data' ? ['--no-create-info', '--skip-triggers', database, table] : [database, table]; return this.dumpWithArgs(connection, output, dumpArgs, processId, false, mode !== 'data'); }
  private dumpWithArgs(connection: DatabaseConnection, output: string, dumpArgs: string[], processId?: string, includeDatabaseObjects = true, includeTriggers = true) { return new Promise<void>((resolve, reject) => { const binary = this.resolveClientBinary(connection.engine, true); const args = [...this.clientToolArgs(binary), `--host=${connection.host}`, `--port=${connection.port}`, `--user=${connection.username}`, '--single-transaction', ...(includeDatabaseObjects ? ['--routines', '--events'] : []), ...(includeTriggers ? ['--triggers'] : []), '--hex-blob', ...dumpArgs]; const child = spawn(binary, args, { env: { ...process.env, MYSQL_PWD: connection.password }, stdio: ['ignore', 'pipe', 'pipe'], windowsHide: true }); const out = fs.createWriteStream(output, { mode: 0o600 }); let stderr = ''; child.stdout.pipe(out); child.stderr.on('data', chunk => { const text = chunk.toString(); stderr += text; if (processId) this.addProcessLog(processId, text); }); child.on('error', error => { out.close(); if (processId) this.addProcessLog(processId, this.clientStartMessage(binary, error)); reject(new Error(this.clientStartMessage(binary, error))); }); child.on('close', code => { out.close(); if (code === 0) resolve(); else reject(new Error(`${binary} failed with code ${code}: ${stderr.slice(0, 800)}`)); }); }); }
