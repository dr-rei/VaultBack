...
function storageDescription(s){const c=s.config||{};return s.type==='local'?c.path||'Local path':s.type==='ftp'?`${c.host||'FTP host'} · ${c.remotePath||'/'}`:s.type==='webdav'?c.url||'WebDAV URL':s.type==='google-drive'?'Google Drive folder':s.type==='s3'?`S3 · ${c.bucket||'bucket'}${c.objectLockRequired?' · Object Lock':''}`:`OneDrive · ${c.remotePath||'root'}`}
...
function authPasswordField(extra='') { return `<div class="field password-field"><label for="auth-password">Password</label><div class="password-input-wrap"><input id="auth-password" name="password" type="password" ${extra}/><button type="button" class="password-toggle" id="auth-password-toggle" aria-label="Show password" aria-pressed="false" title="Show password"><span aria-hidden="true">&#128065;</span></button></div></div>`; }
function initializeAuthPasswordToggle() {
...
function connectRealtime() {
  ...
  closeRealtime();
  const topics = 'processes,backup_runs,sessions,rate_limit,updates,storage_health,downloads,recovery_tests';
  ...
  source.addEventListener('recovery_tests', event => {
    const payload = realtimeJson(event);
    if (!payload || state.role !== 'admin') return;
    state.recovery = payload;
    if (state.currentView === 'recovery') renderRecovery();
  });
  source.onerror = () => {
...
function showAuth(setup){
  ...
  const content=setup?`<h2>Set up your control plane</h2><p>Create the first administrator. This account is required to manage credentials and backup destinations.</p><div class="callout">Your encryption key is generated and kept with the application data directory. Back up that key together with the SQLite file.</div>${field('Admin username','username','','text','autocomplete="username"')}${authPasswordField('autocomplete="new-password" minlength="12"')}<div id="auth-feedback" class="auth-feedback" role="alert" aria-live="assertive" hidden></div><button class="primary" id="auth-submit" type="submit">Create administrator</button><div class="auth-note">Use at least 12 characters. Keep this admin account private.</div>`:`<h2>Welcome back</h2><p>Sign in to manage backup sources, schedules, and destinations.</p>${field('Username','username','','text','autocomplete="username"')}${authPasswordField('autocomplete="current-password"')}<div id="auth-feedback" class="auth-feedback" role="alert" aria-live="assertive" hidden></div><button class="primary" id="auth-submit" type="submit">Sign in</button>`;
  $('#auth-content').innerHTML=`${dependencyNotice()}<form id="auth-form">${content}</form>`;
  initializeAuthPasswordToggle();
  $('#auth-form').onsubmit=async(event)=>{
    ...
    catch(e){const message=e.message||'Unable to continue';if(feedback){feedback.textContent=message;feedback.hidden=false;feedback.classList.add('show');feedback.scrollIntoView?.({block:'nearest'})}toast(message,true);setButtonBusy(button, false)}
  };
  $('#auth-form').addEventListener('input',()=>{const feedback=$('#auth-feedback');if(feedback){feedback.hidden=true;feedback.classList.remove('show')}});
}
...
function showStorageForm(item={}){
  ...
  const common=selectField('Storage type','type',[['local','Local disk'],['ftp','FTP / FTPS'],['webdav','WebDAV / Synology'],['google-drive','Google Drive'],['onedrive','OneDrive'],['s3','S3 / S3-compatible Object Lock']],type);
  ...
  const draw=()=>{const selected=el.querySelector('[name=type]').value;$('#storage-guide-wrap').innerHTML=storageGuide(selected);$('#storage-specific').innerHTML=storageFields(selected,c);const lock=$('#storage-specific [name=objectLockRequired]');if(lock)lock.onchange=()=>{c.objectLockRequired=lock.checked;draw()}};
...
function showJobForm(item = {}) {
  ...
  el.innerHTML = formShell(item.id ? 'Edit schedule' : 'Create schedule',
    ...
    '<div id="job-object-options" class="field full"></div>' +
  ...
  const updateLayoutOptions = () => {
    ...
    const current = {}; for (const [key, name] of Object.entries({ views: 'backupObjectViews', routines: 'backupObjectRoutines', triggers: 'backupObjectTriggers', events: 'backupObjectEvents' })) { const input = el.querySelector(`[name="${name}"]`); if (input) current[key] = input.checked; }
    renderJobObjectOptions(el, layoutSelect?.value || 'single', Object.keys(current).length ? current : (item.backupObjects || {}));
  };
...
const vaultbackViewPaths = { dashboard: '/', connections: '/databases', storage: '/storage', jobs: '/schedules', runs: '/history', recovery: '/recovery', guide: '/guide', settings: '/settings', sessions: '/sessions' };
...
function setView(view, options = {}) {
  ...
  const title = { dashboard: 'Overview', connections: 'Databases', storage: 'Storage targets', jobs: 'Schedules', runs: 'Backup history', recovery: 'Recovery assurance', processes: 'Live processes', guide: 'Setup guide', settings: 'Settings', sessions: 'Sessions & rate limit', 'not-found': 'Page not found' }[nextView];
  $('#section-title').textContent = title;
  $('#section-eyebrow').textContent = { dashboard: 'OPERATIONS CENTER', connections: 'SOURCE DATABASES', storage: 'DELIVERY DESTINATIONS', jobs: 'AUTOMATION', runs: 'AUDIT TRAIL', recovery: 'RESILIENCE CENTER', processes: 'LIVE OPERATIONS', guide: 'HELP CENTER', settings: 'CONTROL PLANE', sessions: 'SECURITY CENTER', 'not-found': 'ERROR 404' }[nextView];
  ...
  if (nextView === 'recovery') { renderRecovery(); if (state.csrf) void loadRecovery(); }
...
function storageFields(type, c) {
  ...
  if (type === 's3') return field('Endpoint (optional)', 'endpoint', c.endpoint || '', 'url', 'placeholder="https://s3.example.com"') + field('Region', 'region', c.region || 'us-east-1') + field('Bucket', 'bucket', c.bucket || '') + field('Prefix', 'prefix', c.prefix || 'vaultback') + field('Access key ID', 'accessKeyId', '', 'text', keep) + field('Secret access key', 'secretAccessKey', '', 'password', keep) + `<label class="field checkbox-field"><span><input name="forcePathStyle" type="checkbox" value="true" ${c.forcePathStyle ? 'checked' : ''} /> Use path-style requests</span></label><label class="field full checkbox-field"><span><input name="objectLockRequired" type="checkbox" value="true" ${c.objectLockRequired ? 'checked' : ''} /> Require provider-enforced Object Lock</span><small class="hint">The bucket must have Object Lock enabled before you save this target. VaultBack will apply COMPLIANCE retention and will never delete objects through rotation.</small></label>${c.objectLockRequired ? field('Retention (days)', 'retentionDays', c.retentionDays || 30, 'number', 'min="1" max="3650"') : ''}`;
...
function storageGuide(type) {
  const guides = {
    ...
    s3: ['S3 / S3-compatible Object Lock', 'Use an S3 bucket for durable object storage. Object Lock is optional, but it is the only provider-enforced immutable mode currently supported by VaultBack.', ['Create the bucket with Object Lock enabled before configuring this target; AWS cannot enable it later on an existing bucket.', 'Grant the access key only bucket-scoped List, Get, Put, and (for mutable targets) Delete permissions.', 'Enter the bucket, region, endpoint if using a compatible provider, and credentials, then click Test.', 'Enable Require provider-enforced Object Lock only when the bucket test confirms Object Lock is enabled.'], 'COMPLIANCE retention cannot be shortened or deleted before the provider retention date. Keep the access key and bucket policy protected.']
  };
  const guide = guides[type] || guides.local;
  const links = type === 'webdav' ? '<p><a href="https://kb.synology.com/en-nz/DSM/tutorial/How_to_access_files_on_Synology_NAS_with_WebDAV" target="_blank" rel="noreferrer">Open Synology WebDAV instructions ↗</a></p>' : type === 'google-drive' ? '<p><a href="https://developers.google.com/identity/protocols/oauth2/web-server" target="_blank" rel="noreferrer">Open Google OAuth instructions ↗</a> · <a href="https://developers.google.com/oauthplayground/" target="_blank" rel="noreferrer">Open OAuth Playground ↗</a></p>' : type === 'onedrive' ? '<p><a href="https://learn.microsoft.com/en-us/entra/identity-platform/v2-oauth2-auth-code-flow" target="_blank" rel="noreferrer">Open Microsoft OAuth instructions ↗</a> · <a href="https://learn.microsoft.com/en-us/graph/permissions-reference" target="_blank" rel="noreferrer">View Graph permissions ↗</a></p>' : type === 's3' ? '<p><a href="https://docs.aws.amazon.com/AmazonS3/latest/userguide/object-lock.html" target="_blank" rel="noreferrer">Open S3 Object Lock documentation ↗</a></p>' : '';
...
function updateRoleUi() {
  ...
  ['new-connection', 'new-storage', 'new-job', 'new-recovery-plan', 'new-recovery-connection'].forEach(id => {
  ...
  document.querySelectorAll('.admin-only-setting').forEach(item => item.classList.toggle('hidden', !canConfigure));
}
...
document.getElementById('new-recovery-plan')?.addEventListener('click', openRecoveryPlanModal);
document.getElementById('new-recovery-connection')?.addEventListener('click', () => openRecoveryConnectionModal());
document.getElementById('refresh-recovery')?.addEventListener('click', () => void withButtonBusy(document.getElementById('refresh-recovery'), loadRecovery, 'Refreshing…'));
document.getElementById('enroll-fleet')?.addEventListener('click', enrollFleetServer);
...
async function loadRecovery() {
  try { state.recovery = await api('/api/recovery/dashboard'); state.recoveryConnections = state.role === 'admin' ? await api('/api/recovery/connections') : []; if (state.role === 'admin') state.recovery.fleet = await api('/api/recovery/fleet'); renderRecovery(); }
  catch (error) { const host = $('#recovery-policy'); if (host) host.innerHTML = `<div class="callout">Unable to load recovery assurance: ${esc(error.message)}</div>`; }
}
function recoveryDuration(seconds) { const value = Number(seconds); if (!Number.isFinite(value)) return '—'; if (value < 60) return `${value}s`; return `${Math.floor(value / 60)}m ${value % 60}s`; }
function renderRecovery() {
  const data = state.recovery || {};
  const summary = data.policy?.summary || {};
  const plans = Array.isArray(data.plans) ? data.plans : [];
  const tests = Array.isArray(data.tests) ? data.tests : [];
  const pitr = data.pitr?.items || [];
  const summaryHost = $('#recovery-summary');
  if (!$('#recovery-destinations-panel') && summaryHost) {
    summaryHost.insertAdjacentHTML('afterend', '<div class="panel admin-only-setting hidden" id="recovery-destinations-panel"><div class="panel-head"><div><span class="kicker">ISOLATED DESTINATIONS</span><h3>Recovery test servers</h3></div><button type="button" class="primary" id="new-recovery-connection">+ Add recovery server</button></div><p class="hint">Configure a dedicated database server for restore tests. These encrypted credentials are not shown in Databases or Schedules.</p><div id="recovery-connections-list" class="recovery-connections-list"></div></div>');
    $('#new-recovery-connection')?.addEventListener('click', () => openRecoveryConnectionModal());
  }
  if (summaryHost) summaryHost.innerHTML = `<article class="metric"><span class="metric-label">Recovery plans</span><strong>${plans.length}</strong><small>Scheduled restore rehearsals</small></article><article class="metric ${summary.high || summary.critical ? 'metric-warning' : ''}"><span class="metric-label">Needs attention</span><strong>${Number(summary.critical || 0) + Number(summary.high || 0) + Number(summary.medium || 0)}</strong><small>${Number(summary.ready || 0)} policies ready</small></article><article class="metric"><span class="metric-label">Passed tests</span><strong>${tests.filter(item => item.status === 'passed').length}</strong><small>Evidence recorded locally</small></article><article class="metric ${pitr.some(item => item.status === 'ready') ? 'accent' : ''}"><span class="metric-label">PITR readiness</span><strong>${pitr.filter(item => item.status === 'ready').length}/${pitr.length || 0}</strong><small>Connections with binary logs</small></article>`;
  const policyHost = $('#recovery-policy');
  if (policyHost) policyHost.innerHTML = (data.policy?.items || []).length ? data.policy.items.map(item => `<article class="recovery-policy-item risk-${esc(item.risk)}"><div><b>${esc(item.jobName)}</b><small>${esc(item.storageName || 'No storage target')} · ${item.latestBackupAt ? `Last success ${esc(fmtDate(item.latestBackupAt))}` : 'No successful backup yet'}</small></div><span class="tag">${esc(item.risk)}</span><div class="recovery-findings">${item.findings.length ? item.findings.map(finding => `<div><b>${esc(finding.title)}</b><span>${esc(finding.detail)}</span></div>`).join('') : '<span class="success-text">No policy findings</span>'}</div></article>`).join('') : '<div class="empty">No schedules are configured yet.</div>';
  const pitrHost = $('#recovery-pitr');
  if (pitrHost) pitrHost.innerHTML = pitr.length ? pitr.map(item => `<article class="recovery-pitr-item"><div class="recovery-item-heading"><div><b>${esc(item.name)}</b><small>${esc(item.engine || '')} · ${Number(item.artifactCount || 0)} captured artifact(s)</small></div><span class="tag ${item.status === 'ready' ? 'tag-success' : 'tag-danger'}">${esc(item.status || 'unknown')}</span></div><p>${esc(item.message || 'Not checked')}</p>${item.latestLog ? `<small>Binlog range: ${esc(item.earliestLog || '—')} → ${esc(item.latestLog)}</small>` : ''}${item.lastCaptureAt ? `<small>Last capture: ${esc(fmtDate(item.lastCaptureAt))}</small>` : ''}${state.role === 'admin' && item.status === 'ready' ? `<div class="recovery-item-actions"><button type="button" class="small-button" onclick="openPitrCaptureModal('${esc(item.connectionId)}','${esc(item.name)}')">Capture current binlogs</button></div>` : ''}</article>`).join('') : '<div class="empty">Add a database connection to inspect point-in-time recovery.</div>';
  const planHost = $('#recovery-plans');
  if (planHost) planHost.innerHTML = plans.length ? plans.map(plan => `<article class="recovery-plan-item"><div class="recovery-item-heading"><div><b>${esc(plan.name)}</b><small>${esc(plan.jobName)} → ${esc(plan.destinationName)} · ${esc(plan.cronExpression)} ${esc(plan.timezone)}</small></div><span class="tag ${plan.lastStatus === 'passed' ? 'tag-success' : plan.lastStatus === 'failed' ? 'tag-danger' : ''}">${esc(plan.lastStatus || 'not tested')}</span></div><p>${esc(plan.lastMessage || 'No restore evidence has been recorded yet.')}</p><div class="recovery-item-actions"><button type="button" class="small-button" onclick="runRecoveryTest('${esc(plan.id)}')">Run test</button><button type="button" class="small-button danger-text" onclick="deleteRecoveryPlan('${esc(plan.id)}')">Delete</button></div></article>`).join('') : '<div class="empty">No restore plans yet. Add one to prove a backup can be recovered.</div>';
  const testHost = $('#recovery-tests');
  if (testHost) testHost.innerHTML = tests.length ? tests.slice(0, 20).map(test => `<article class="recovery-test-item"><div><b>${esc(test.planName || 'Recovery test')}</b><small>${esc(test.filename || 'Backup artifact unavailable')} · ${esc(fmtDate(test.startedAt))}</small></div><span class="tag ${test.status === 'passed' ? 'tag-success' : test.status === 'failed' ? 'tag-danger' : ''}">${esc(test.status)}</span><div><small>RPO ${recoveryDuration(test.rpoSeconds)} · RTO ${recoveryDuration(test.rtoSeconds)}</small>${test.errorMessage ? `<p class="danger-text">${esc(test.errorMessage)}</p>` : ''}</div></article>`).join('') : '<div class="empty">No recovery-test evidence yet.</div>';
  const runbookHost = $('#recovery-runbooks');
  if (runbookHost) runbookHost.innerHTML = (data.runbooks || []).map(book => `<details class="recovery-runbook"><summary><b>${esc(book.title)}</b></summary><ol>${(book.steps || []).map(step => `<li>${esc(step)}</li>`).join('')}</ol></details>`).join('');
  const fleetHost = $('#recovery-fleet-list');
  if (fleetHost) fleetHost.innerHTML = (data.fleet || []).length ? data.fleet.map(server => `<article class="recovery-fleet-item"><div><b>${esc(server.name)}</b><small>${esc(server.installationId)} · ${esc(server.lastSeenAt ? fmtDate(server.lastSeenAt) : 'Never seen')}</small></div><span class="tag ${server.status === 'active' ? 'tag-success' : 'tag-danger'}">${esc(server.status)}</span>${server.status === 'active' ? `<button type="button" class="small-button danger-text" onclick="revokeFleetServer('${esc(server.id)}')">Revoke</button>` : ''}</article>`).join('') : '<div class="empty">No installations enrolled.</div>';
  const recoveryConnectionsHost = $('#recovery-connections-list');
  if (recoveryConnectionsHost) recoveryConnectionsHost.innerHTML = state.role === 'admin' && state.recoveryConnections.length ? state.recoveryConnections.map(connection => `<article class="recovery-connection-item"><div><b>${esc(connection.name)}</b><small>${esc(connection.engine)} · ${esc(connection.host)}:${esc(connection.port)} · ${esc(connection.username)}${connection.ssl ? ' · TLS' : ''}</small></div><div class="recovery-item-actions"><button type="button" class="small-button" onclick="openRecoveryConnectionModal('${esc(connection.id)}')">Edit</button><button type="button" class="small-button danger-text" onclick="deleteRecoveryConnection('${esc(connection.id)}')">Delete</button></div></article>`).join('') : '<div class="empty">Add a dedicated database server to run isolated restore tests. It will stay hidden from Databases and Schedules.</div>';
}
function closeRecoveryModal() { $('#recovery-plan-modal')?.remove(); document.body.classList.remove('modal-open'); }
function closeRecoveryConnectionModal() { $('#recovery-connection-modal')?.remove(); document.body.classList.remove('modal-open'); }
function openRecoveryConnectionModal(id = '') {
  if (state.role !== 'admin') return toast('Only administrators can manage recovery servers', true);
  const existing = state.recoveryConnections.find(connection => connection.id === id) || {};
  const root = $('#modal-root'); if (!root) return;
  root.insertAdjacentHTML('beforeend', `<section id="recovery-connection-modal" class="form-panel recovery-modal" role="dialog" aria-modal="true" aria-labelledby="recovery-connection-modal-title"><form class="form-shell" onsubmit="return false"><button type="button" class="modal-close" aria-label="Close recovery server dialog" onclick="closeRecoveryConnectionModal()">×</button><span class="kicker">ISOLATED DESTINATION</span><h3 id="recovery-connection-modal-title">${id ? 'Edit recovery test server' : 'Add recovery test server'}</h3><p class="hint">Use a separate database host for restore rehearsals. VaultBack encrypts this password and never returns it to the browser.</p><div class="form-grid"><div class="field full"><label for="recovery-connection-name">Display name</label><input id="recovery-connection-name" value="${esc(existing.name || '')}" placeholder="Disaster recovery MySQL" autocomplete="off" /></div><div class="field"><label for="recovery-connection-engine">Engine</label><select id="recovery-connection-engine"><option value="mysql" ${existing.engine === 'mysql' || !existing.engine ? 'selected' : ''}>MySQL</option><option value="mariadb" ${existing.engine === 'mariadb' ? 'selected' : ''}>MariaDB</option></select></div><div class="field"><label for="recovery-connection-host">Host</label><input id="recovery-connection-host" value="${esc(existing.host || '')}" placeholder="10.0.20.15" autocomplete="off" /></div><div class="field"><label for="recovery-connection-port">Port</label><input id="recovery-connection-port" type="number" min="1" max="65535" value="${esc(existing.port || 3306)}" /></div><div class="field"><label for="recovery-connection-username">Username</label><input id="recovery-connection-username" value="${esc(existing.username || '')}" autocomplete="username" /></div><div class="field full"><label for="recovery-connection-password">Password <span class="optional">Optional</span></label><input id="recovery-connection-password" type="password" autocomplete="new-password" placeholder="${id ? 'Leave blank to keep current password' : 'Leave blank for passwordless login'}" /></div><div class="field full"><label class="checkbox-label"><input id="recovery-connection-ssl" type="checkbox" ${existing.ssl ? 'checked' : ''} /> Use TLS</label></div></div><div id="recovery-connection-status" class="callout hidden" aria-live="polite"></div><div class="form-actions"><button type="button" class="secondary" onclick="closeRecoveryConnectionModal()">Cancel</button><button type="button" class="secondary" id="test-recovery-connection">Test connection</button><button type="button" class="primary" id="save-recovery-connection">Save server</button></div></form></section>`);
  document.body.classList.add('modal-open');
  const formBody = () => ({ id: id || undefined, name: $('#recovery-connection-name').value.trim(), engine: $('#recovery-connection-engine').value, host: $('#recovery-connection-host').value.trim(), port: Number($('#recovery-connection-port').value || 3306), username: $('#recovery-connection-username').value.trim(), password: $('#recovery-connection-password').value, ssl: $('#recovery-connection-ssl').checked });
  const showStatus = (message, danger = false) => { const status = $('#recovery-connection-status'); if (status) { status.textContent = message; status.classList.remove('hidden'); status.classList.toggle('danger-callout', danger); } };
  $('#test-recovery-connection').onclick = async () => { try { await withButtonBusy($('#test-recovery-connection'), async () => { const result = await api('/api/recovery/connections/test', { method: 'POST', body: JSON.stringify(formBody()) }); if (!result.ok) throw new Error(result.message || 'Database connection failed'); showStatus(result.message || 'Database connection successful'); toast('Recovery server connection successful'); }, 'Testing…'); } catch (error) { showStatus(error.message, true); toast(error.message, true); } };
  $('#save-recovery-connection').onclick = async () => { try { await withButtonBusy($('#save-recovery-connection'), async () => { const body = formBody(); if (!body.name || !body.host || !body.username) throw new Error('Name, host, and username are required'); await api('/api/recovery/connections', { method: 'POST', body: JSON.stringify(body) }); closeRecoveryConnectionModal(); await loadRecovery(); toast('Recovery test server saved'); }, 'Saving…'); } catch (error) { showStatus(error.message, true); toast(error.message, true); } };
}
async function deleteRecoveryConnection(id) { if (!await appDialog({ title: 'Delete recovery test server?', message: 'This server can be deleted only when no recovery plan uses it. Backup files and normal database connections are not affected.', confirmText: 'Delete server', cancelText: 'Cancel', danger: true })) return; try { await api(`/api/recovery/connections/${encodeURIComponent(id)}`, { method: 'DELETE' }); toast('Recovery test server deleted'); await loadRecovery(); } catch (error) { toast(error.message, true); } }
function openRecoveryPlanModal() {
  if (state.role !== 'admin') return toast('Only administrators can create recovery plans', true);
  const root = $('#modal-root'); if (!root) return;
  const jobs = state.jobs.map(job => `<option value="${esc(job.id)}">${esc(job.name)}</option>`).join('');
  if (!state.recoveryConnections.length) return toast('Add a dedicated recovery server before creating a restore test', true);
  const connections = state.recoveryConnections.map(connection => `<option value="${esc(connection.id)}">${esc(connection.name)} · ${esc(connection.host)}</option>`).join('');
  root.insertAdjacentHTML('beforeend', `<section id="recovery-plan-modal" class="form-panel recovery-modal" role="dialog" aria-modal="true" aria-labelledby="recovery-plan-modal-title"><form class="form-shell" onsubmit="return false"><button type="button" class="modal-close" aria-label="Close recovery plan dialog" onclick="closeRecoveryModal()">×</button><span class="kicker">RESTORE REHEARSAL</span><h3 id="recovery-plan-modal-title">Add restore test plan</h3><p class="hint">VaultBack will use the latest successful backup, restore it into a generated disposable database, verify it, and remove that database.</p><div class="form-grid"><div class="field full"><label for="recovery-plan-name">Plan name</label><input id="recovery-plan-name" placeholder="Weekly production restore test" /></div><div class="field"><label for="recovery-plan-job">Backup schedule</label><select id="recovery-plan-job">${jobs}</select></div><div class="field"><label for="recovery-plan-connection">Recovery test server</label><select id="recovery-plan-connection">${connections}</select></div><div class="field"><label for="recovery-plan-cron">Test schedule</label><input id="recovery-plan-cron" value="0 4 * * 0" /></div><div class="field"><label for="recovery-plan-timezone">Timezone</label><input id="recovery-plan-timezone" value="UTC" /></div><div class="field full"><label for="recovery-plan-prefix">Temporary database prefix</label><input id="recovery-plan-prefix" value="vaultback_recovery_test" maxlength="40" /></div></div><div class="form-actions"><button type="button" class="secondary" onclick="closeRecoveryModal()">Cancel</button><button type="button" class="primary" id="save-recovery-plan">Save plan</button></div></form></section>`);
  document.body.classList.add('modal-open');
  $('#save-recovery-plan').onclick = async () => { const button = $('#save-recovery-plan'); try { await withButtonBusy(button, async () => { await api('/api/recovery/plans', { method: 'POST', body: JSON.stringify({ name: $('#recovery-plan-name').value.trim(), jobId: $('#recovery-plan-job').value, recoveryConnectionId: $('#recovery-plan-connection').value, cronExpression: $('#recovery-plan-cron').value.trim(), timezone: $('#recovery-plan-timezone').value.trim(), testDatabasePrefix: $('#recovery-plan-prefix').value.trim(), enabled: true }) }); closeRecoveryModal(); await loadRecovery(); toast('Recovery plan saved'); }, 'Saving…'); } catch (error) { toast(error.message, true); } };
}
async function runRecoveryTest(id) { const button = document.querySelector(`button[onclick="runRecoveryTest('${id}')"]`); try { await withButtonBusy(button, () => api(`/api/recovery/plans/${encodeURIComponent(id)}/run`, { method: 'POST' }), 'Testing…'); toast('Recovery test completed; evidence updated'); await loadRecovery(); } catch (error) { toast(error.message, true); await loadRecovery(); } }
function closePitrCaptureModal() { $('#pitr-capture-modal')?.remove(); document.body.classList.remove('modal-open'); }
function openPitrCaptureModal(connectionId, connectionName) { const targets = state.storage.filter(target => ['local', 'ftp', 'webdav', 'google-drive', 'onedrive', 's3'].includes(target.type)); if (!targets.length) return toast('Add a storage target before capturing binary logs', true); const root = $('#modal-root'); root.insertAdjacentHTML('beforeend', `<section id="pitr-capture-modal" class="form-panel recovery-modal" role="dialog" aria-modal="true" aria-labelledby="pitr-capture-modal-title"><form class="form-shell" onsubmit="return false"><button type="button" class="modal-close" aria-label="Close PITR capture dialog" onclick="closePitrCaptureModal()">×</button><span class="kicker">PITR CAPTURE</span><h3 id="pitr-capture-modal-title">Capture binary logs</h3><p class="hint">Capture the currently available raw binlog files for <b>${esc(connectionName)}</b>. This uploads evidence only; it does not restore or apply logs.</p><div class="field full"><label for="pitr-storage-target">Storage target</label><select id="pitr-storage-target">${targets.map(target => `<option value="${esc(target.id)}">${esc(target.name)} · ${esc(target.type)}${target.type === 's3' && target.config?.objectLockRequired ? ' · Object Lock' : ''}</option>`).join('')}</select></div><div class="callout">Use an S3 Object Lock target for tamper-resistant retention. Other providers can store the files, but VaultBack cannot claim provider-enforced immutability for them.</div><div class="form-actions"><button type="button" class="secondary" onclick="closePitrCaptureModal()">Cancel</button><button type="button" class="primary" id="capture-pitr">Capture and upload</button></div></form></section>`); document.body.classList.add('modal-open'); $('#capture-pitr').onclick = async () => { try { await withButtonBusy($('#capture-pitr'), () => api('/api/recovery/pitr/capture', { method: 'POST', body: JSON.stringify({ connectionId, storageTargetId: $('#pitr-storage-target').value }) }), 'Capturing…'); closePitrCaptureModal(); toast('Raw binary logs captured and uploaded'); await loadRecovery(); } catch (error) { toast(error.message, true); } }; }
async function deleteRecoveryPlan(id) { if (!await appDialog({ title: 'Delete recovery plan?', message: 'Existing evidence will also be removed. Backup files and database data are not deleted.', confirmText: 'Delete plan', cancelText: 'Cancel', danger: true })) return; try { await api(`/api/recovery/plans/${encodeURIComponent(id)}`, { method: 'DELETE' }); toast('Recovery plan deleted'); await loadRecovery(); } catch (error) { toast(error.message, true); } }
async function enrollFleetServer() { try { await withButtonBusy($('#enroll-fleet'), async () => { await api('/api/recovery/fleet/enroll', { method: 'POST', body: JSON.stringify({ name: $('#fleet-name').value.trim(), installationId: $('#fleet-installation').value.trim(), token: $('#fleet-token').value }) }); $('#fleet-name').value = ''; $('#fleet-installation').value = ''; $('#fleet-token').value = ''; toast('Server enrolled'); await loadRecovery(); }, 'Enrolling…'); } catch (error) { toast(error.message, true); } }
async function revokeFleetServer(id) { if (!await appDialog({ title: 'Revoke this server?', message: 'The installation will no longer be treated as active in the fleet registry.', confirmText: 'Revoke server', cancelText: 'Cancel', danger: true })) return; try { await api(`/api/recovery/fleet/${encodeURIComponent(id)}/revoke`, { method: 'POST' }); toast('Server revoked'); await loadRecovery(); } catch (error) { toast(error.message, true); } }

async function loadAll() {
  await Promise.all(['connections', 'storage', 'jobs', 'runs'].map(loadCollection).concat(loadBackupActivity(), loadRecovery()));
