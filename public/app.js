...
async function loadUpdateInfo(check = false) { if (state.role !== 'admin') return; ensureUpdatePanel(); try { state.update = await api(check ? '/api/settings/update/check' : '/api/settings/update'); renderUpdatePanel(); } catch (e) { state.update = { status: 'failed', error: e.message }; renderUpdatePanel(); } }
