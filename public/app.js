...
function updateRoleUi() {
  ...
  ['new-connection', 'new-storage', 'new-job', 'new-recovery-plan'].forEach(id => {
...
async function loadRecovery() {
  try { state.recovery = await api('/api/recovery/dashboard'); if (state.role === 'admin') state.recovery.fleet = await api('/api/recovery/fleet'); renderRecovery(); }
...
function renderRecovery() {
  ...
  if (pitrHost) pitrHost.innerHTML = pitr.length ? pitr.map(item => `<article class="recovery-pitr-item"><div><b>${esc(item.name)}</b><small>${esc(item.engine || '')} · ${Number(item.artifactCount || 0)} captured artifact(s)</small></div><span class="tag ${item.status === 'ready' ? 'tag-success' : 'tag-danger'}">${esc(item.status || 'unknown')}</span><p>${esc(item.message || 'Not checked')}</p>${item.latestLog ? `<small>Binlog range: ${esc(item.earliestLog || '—')} → ${esc(item.latestLog)}</small>` : ''}${item.lastCaptureAt ? `<small>Last capture: ${esc(fmtDate(item.lastCaptureAt))}</small>` : ''}${state.role === 'admin' && item.status === 'ready' ? `<button type="button" class="small-button" onclick="openPitrCaptureModal('${esc(item.connectionId)}','${esc(item.name)}')">Capture current binlogs</button>` : ''}</article>`).join('') : '<div class="empty">Add a database connection to inspect point-in-time recovery.</div>';
  const planHost = $('#recovery-plans');
  if (planHost) planHost.innerHTML = plans.length ? plans.map(plan => `<article class="recovery-plan-item"><div><b>${esc(plan.name)}</b><small>${esc(plan.jobName)} → ${esc(plan.destinationName)} · ${esc(plan.cronExpression)} ${esc(plan.timezone)}</small></div><div class="recovery-plan-state"><span class="tag ${plan.lastStatus === 'passed' ? 'tag-success' : plan.lastStatus === 'failed' ? 'tag-danger' : ''}">${esc(plan.lastStatus || 'not tested')}</span><button type="button" class="small-button" onclick="runRecoveryTest('${esc(plan.id)}')">Run test</button><button type="button" class="small-button danger-text" onclick="deleteRecoveryPlan('${esc(plan.id)}')">Delete</button></div><p>${esc(plan.lastMessage || 'No restore evidence has been recorded yet.')}</p></article>`).join('') : '<div class="empty">No restore plans yet. Add one to prove a backup can be recovered.</div>';
...
function openRecoveryPlanModal() {
  ...
  const connections = state.connections.map(connection => `<option value="${esc(connection.id)}">${esc(connection.name)} · ${esc(connection.host)}</option>`).join('');
  root.insertAdjacentHTML('beforeend', `<section id="recovery-plan-modal" class="form-panel"><form class="form-shell" onsubmit="return false"><button type="button" class="modal-close" aria-label="Close recovery plan dialog" onclick="closeRecoveryModal()">×</button><h3>Add restore test plan</h3><p class="hint">VaultBack will use the latest successful backup, restore it into a generated disposable database, verify it, and remove that database.</p><div class="form-grid"><div class="field full"><label for="recovery-plan-name">Plan name</label><input id="recovery-plan-name" placeholder="Weekly production restore test" /></div><div class="field"><label for="recovery-plan-job">Backup schedule</label><select id="recovery-plan-job">${jobs}</select></div><div class="field"><label for="recovery-plan-connection">Isolated destination</label><select id="recovery-plan-connection">${connections}</select></div><div class="field"><label for="recovery-plan-cron">Test schedule</label><input id="recovery-plan-cron" value="0 4 * * 0" /></div><div class="field"><label for="recovery-plan-timezone">Timezone</label><input id="recovery-plan-timezone" value="UTC" /></div><div class="field full"><label for="recovery-plan-prefix">Temporary database prefix</label><input id="recovery-plan-prefix" value="vaultback_recovery_test" maxlength="40" /></div></div><div class="form-actions"><button type="button" class="secondary" onclick="closeRecoveryModal()">Cancel</button><button type="button" class="primary" id="save-recovery-plan">Save plan</button></div></form></section>`);
  document.body.classList.add('modal-open');
  $('#save-recovery-plan').onclick = async () => { const button = $('#save-recovery-plan'); try { await withButtonBusy(button, async () => { await api('/api/recovery/plans', { method: 'POST', body: JSON.stringify({ name: $('#recovery-plan-name').value.trim(), jobId: $('#recovery-plan-job').value, destinationConnectionId: $('#recovery-plan-connection').value, cronExpression: $('#recovery-plan-cron').value.trim(), timezone: $('#recovery-plan-timezone').value.trim(), testDatabasePrefix: $('#recovery-plan-prefix').value.trim(), enabled: true }) }); closeRecoveryModal(); await loadRecovery(); toast('Recovery plan saved'); }, 'Saving…'); } catch (error) { toast(error.message, true); } };
}
