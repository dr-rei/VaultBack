...
function showAuth(setup){
  ...
  const content=setup?`<h2>Set up your control plane</h2><p>Create the first administrator. This account is required to manage credentials and backup destinations.</p><div class="callout">Your encryption key is generated and kept with the application data directory. Back up that key together with the SQLite file.</div>${field('Admin username','username','','text','autocomplete="username"')}${field('Password','password','','password','autocomplete="new-password" minlength="12"')}<div id="auth-feedback" class="auth-feedback" role="alert" aria-live="assertive" hidden></div><button class="primary" id="auth-submit" type="submit">Create administrator</button><div class="auth-note">Use at least 12 characters. Keep this admin account private.</div>`:`<h2>Welcome back</h2><p>Sign in to manage backup sources, schedules, and destinations.</p>${field('Username','username','','text','autocomplete="username"')}${field('Password','password','','password','autocomplete="current-password"')}<div id="auth-feedback" class="auth-feedback" role="alert" aria-live="assertive" hidden></div><button class="primary" id="auth-submit" type="submit">Sign in</button>`;
