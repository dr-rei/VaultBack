...
function showJobForm(item = {}) {
  ...
  el.innerHTML = formShell(item.id ? 'Edit schedule' : 'Create schedule',
    ...
    `<div id="table-schema-options" class="field full ${item.backupLayout === 'table' ? '' : 'hidden'}"><div class="settings-checks"><label><input name="tableSchemaSeparate" type="checkbox" ${item.tableSchemaSeparate ? 'checked' : ''}> Separate table schema into its own SQL file</label></div><span class="hint">Creates <code>TableName.schema.sql</code> and <code>TableName.data.sql</code> inside each database folder.</span></div>` +
  ...
  const schemaOptions = el.querySelector('#table-schema-options');
  ...
  const updateLayoutOptions = () => {
    ...
    schemaOptions?.classList.toggle('hidden', layoutSelect?.value !== 'table');
  ...
  $('#save-form').onclick = async () => {
    try {
      ...
      data.tableSchemaSeparate = form.querySelector('[name="tableSchemaSeparate"]')?.checked === true;
